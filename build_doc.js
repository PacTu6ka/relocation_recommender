...
const intro = [
    ...
    H2("1.1. Проблема"),
    P("Выбор страны для переезда — многомерная задача, в которой пользователь должен балансировать десятки факторов: стоимость жизни, безопасность, качество медицины, простоту визового режима, климат, языковой барьер и др. Существующие источники (Numbeo, Nomad List) дают лишь сырые рейтинги, не учитывая индивидуальные приоритеты."),

    H2("1.2. Цель проекта"),
    P("Построить рабочий end-to-end ML-пайплайн, который:"),
    bullet("Собирает данные о странах из открытых источников (87 стран × 20 признаков)"),
    bullet("Кластеризует страны по социально-экономическим характеристикам"),
    bullet("Принимает структурированный профиль пользователя (бюджет, приоритеты, языки)"),
    ...
    makeTable(
        [
            ...
            ["Hit Rate (идеал в топ-10)", "96.0 %", "48 из 50 синтетических профилей"],
            ["NDCG@10 baseline", "0.7906", "Косинусная модель без RL"],
            ["NDCG@10 LinUCB", "0.3491*", "На сложном тест-сете, +27.11 % vs base"],
            ["Coverage", "87.4 %", "76 из 87 стран попадают в рекомендации"],
            ["Diversity@10", "0.046", "Умеренная разнородность"],
            ["Стран в датасете", "87", "Из 238 исходных (фильтр >40% NA)"],
            ["Признаков", "20", "После MinMax-нормализации"],
        ],
        [3200, 2200, 4000]
    ),
    P("* Тест-сет в evaluate.py и rl_agent.py разный (50 vs 200 профилей, разные распределения); ниже разбираем подробнее.", { run: { italics: true, color: MUTED, size: 20 } }),
];
...
const preprocessing = [
    ...
    makeTable(
        [
            ...
            ["Колонок, прошедших MinMax-scale", "16"],
            ["Boolean-колонок (остались 0/1)", "4"],
        ],
        [5800, 3600]
    ),

    H2("4.5. Допущения"),
    quote("Порог 40% пропусков — компромисс. Меньше → теряем интересные страны (Iran, Bolivia). Больше → импутация слишком агрессивна. 40% даёт 87 стран — достаточно для кластеризации (~12 стран на кластер при K=7)."),
];
...
const recommender = [
    ...
    P("Валидация диапазона 1-5 происходит в __post_init__, языки нормализуются к нижнему регистру."),
    ...
    makeTable(
        [
            ...
            ["2. Инверсия", "residency_difficulty, cost, pollution → 1 - x (выше = лучше)"],
            ...
            ["5. Бонусы", "+0.10 для лучшего кластера, +0.05 язык, +0.03 климат"],
            ["6. Сортировка", "Top-N по итоговому score"],
        ],
        [3000, 6400]
    ),
...
const ui = [
    ...
    bullet("🛡️ Безопасность 1-5"),
    bullet("☀️ Тёплый климат 1-5 (≥3 → True)"),
    bullet("🛂 Лёгкость визы 1-5"),
    bullet("🏥 Медицина 1-5"),
    bullet("Мультиселект знакомых языков (18 опций)"),
    bullet("Слайдер Top-N (5-30)"),

    H2("8.2. Основная область"),

    H3("🗺️ Choropleth-карта мира"),
    P("Plotly choropleth с цветовой шкалой от красного (низкий score) до зелёного (высокий). Диапазон color-axis обрезан по 5-95 перцентилям — иначе цвета \"сжимаются\" из-за выбросов. Hover показывает: страна, score, ключевые метрики. Клик по точке → выбор страны для детального анализа."),

    H3("🏆 Таблица топ-N"),
    P("Колонки: флаг-эмодзи, ISO3, страна, score, стоимость, безопасность, сложность визы, ЕС, DNV, климат. Флаги генерируются из ISO2 через региональные индикаторы Unicode. Строки кликабельны (selection_mode=\"single-row\")."),

    H3("📊 Radar chart"),
    P("Plotly Scatterpolar по 8 осям: Безопасность, Здравоохранение, Доступность, Долголетие, Покупательная способность, Простота визы, ВВП, Экология. Колонки \"чем ниже = лучше\" (cost, residency_difficulty, pollution) инвертируются перед построением."),

    H3("💡 Блок \"Почему эта страна?\""),
    P("Вызывает recommender.explain() и отображает результат через st.code() — моноширинный шрифт сохраняет выравнивание ASCII-баров."),

    H2("8.3. Состояние и интерактивность"),
    P("Выбор страны хранится в st.session_state[\"selected_iso3\"]. Два источника обновляют его независимо:"),
    bullet("Клик по карте: map_event.selection.points[0][\"location\"]"),
    bullet("Клик по строке таблицы: table_event.selection.rows[0]"),
    P("Оба используют on_select=\"rerun\" (доступно с Streamlit 1.33+). Кэширование через @st.cache_data: пересчёт рекомендаций только при изменении параметров профиля."),
...
const evaluation = [
    ...
    makeTable(
        [
            ["Метрика", "Значение"],
            ["Hit Rate (идеал в топ-10)", "96.0 % (48/50)"],
            ["NDCG@10 (все)", "0.7906"],
            ["NDCG@10 (чистые)", "0.7775"],
            ["NDCG@10 (шумные)", "0.8037"],
            ["Coverage", "87.4 % (76/87)"],
            ["Diversity@10", "0.0456"],
        ],
        [5800, 3600]
    ...
    P("Из 48 попаданий: 29 на 1 месте (60 %), 13 на 2 месте, остальные 6 — позиции 3-10."),
    ...
    quote("Интересный артефакт: шумные профили дали NDCG чуть выше чистых (0.8037 vs 0.7775). Гипотеза: шум \"смягчает\" острые веса, что иногда улучшает косинусное сходство."),
];
...
const conclusion = [
    H1("12. Итоги и дальнейшие улучшения"),

    H2("12.1. Что получилось"),
    bullet([
